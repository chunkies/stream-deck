...
import type { Config, ServerInfo, DeckEvent, PluginMeta, PluginManifest, PluginRegistry, UpdateInfo, ProgressData, AppUpdateInfo, PluginLogEntry } from '../shared/types'
...
ipcMain.handle('mp:check-plugin-updates',  ()                  => { if (!pluginsPath) return []; return installer.checkUpdates(pluginsPath, app.getVersion()) })
...
// ── Plugin logs IPC ────────────────────────────────────
ipcMain.handle('mp:get-plugin-logs', (_, pluginId?: string) => server.getPluginLogs(pluginId))
