...
const { semverGt, getInstalledManifests, installPlugin } = require('../server/plugin-installer')
const fs   = require('fs')
...
describe('installPlugin — manifest id validation', () => {
  ...
  test('rejects when manifest.id does not match pluginId', async () => {
    // Build a zip where manifest.id = 'other-id'
    ...
    // Serve the zip locally via a data URL isn't possible; instead mock fetch
