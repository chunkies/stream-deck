...
import { state, adminPages, adminIdx, setAdminIdx } from '../../renderer/src/state'

import type { Component } from '../../shared/types'

const makePage  = (id: string) => ({ id, name: id, components: [] as Component[] })
const makeFolderComp = (pages: ReturnType<typeof makePage>[]): Component => ({ id: 'fc1', componentType: 'folder', col: 0, row: 0, colSpan: 1, rowSpan: 1, pages } as Component)
...
describe('adminPages', () => {
  ...
  test('returns innermost folder pages when nested two levels deep', () => {
    ...
    const outerFolder = makeFolderComp([makePage('outer')])
