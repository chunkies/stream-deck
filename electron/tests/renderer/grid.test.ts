...
describe('renderGrid — component type markup', () => {
  ...
  test('tile with plugin subscription — renders plugin badge and event', () => {
    state.config!.pages[0].components = [makeComp({ componentType: 'tile', pluginTileId: 'my-plugin', pluginTileEvent: 'update', label: 'Plugin' })]
    renderGrid()
    const card = document.querySelector('.comp-card')!
    expect(card.querySelector('.cell-type-badge')!.textContent).toBe('plugin')
  ...
  test('tile without plugin subscription — renders tile badge and command', () => {
    state.config!.pages[0].components = [makeComp({ componentType: 'tile', pollCommand: 'uptime', label: 'Uptime' })]
    renderGrid()
    const card = document.querySelector('.comp-card')!
    expect(card.querySelector('.cell-type-badge')!.textContent).toBe('tile')
    expect(card.querySelector('.cell-tile-cmd')!.textContent).toBe('uptime')
    expect(card.querySelector('.cell-label')!.textContent).toBe('Uptime')
  })
