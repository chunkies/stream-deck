...
const { migrateConfig, validateConfig } = _require('../../../dist/electron/server/config')
const { TIMINGS }                       = _require('../../../dist/electron/server/constants')
...
describe('TIMINGS constants', () => {
  ...
  test('all expected keys are present and positive', () => {
    const required = ['PLUGIN_TIMEOUT_MS', 'TILE_POLL_MIN_MS', 'TILE_POLL_CMD_MS', 'COMMAND_TIMEOUT_MS', 'SEQUENCE_DEFAULT_MS']
