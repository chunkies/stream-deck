...
import type { PluginManifest, PluginRegistry, RegistryPlugin, ProgressData } from '../../shared/types'
...
async function downloadFile(url: string, destPath: string, onProgress: (pct: number) => void): Promise<void> {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      ...
      lastError = err
      if (reader) reader.cancel(undefined).catch(() => {})
    }
  }
...
export async function installPlugin(pluginId: string, downloadUrl: string, pluginsDir: string, onProgress?: ProgressFn, appVersion?: string): Promise<PluginManifest> {
  ...
  const tmpZip     = path.join(pluginsDir, `_tmp_${pluginId}_${suffix}.zip`)
  const tmpExtract = path.join(pluginsDir, `_tmp_extract_${pluginId}_${suffix}`)
  ...
  try {
    ...
    const packageJsonPath = path.join(tmpExtract, 'package.json')
    if (fs.existsSync(packageJsonPath)) {
      if (onProgress) onProgress({ status: 'installing', pct: 50 })
      const heartbeat = setInterval(() => onProgress?.({ status: 'installing', pct: 50 }), 2000)
      try {
        await new Promise<void>((resolve, reject) => {
          execFile(NPM, ['install', '--omit=dev', '--no-audit', '--no-fund'], { cwd: tmpExtract, timeout: 120000 }, (err) => {
    ...
    // Atomic swap: back up existing dir, rename temp extract to final location
    ...
    if (backupMade && fs.existsSync(backupDir)) {
      try { fs.rmSync(backupDir, { recursive: true }) } catch (e) {
        console.error('Failed to remove backup dir after successful install:', e)
      }
    }
    ...
    // Restore backup if extraction/swap failed
    if (backupMade && fs.existsSync(backupDir)) {
      try {
        if (fs.existsSync(destDir)) fs.rmSync(destDir, { recursive: true })
        fs.renameSync(backupDir, destDir)
      } catch (restoreErr) {
        console.error('Failed to restore backup after install failure:', restoreErr)
      }
    }
    // Clean up failed temp extract dir
    if (fs.existsSync(tmpExtract)) {
      try { fs.rmSync(tmpExtract, { recursive: true }) } catch (e) {
        console.error('Failed to remove temp extract dir:', e)
      }
    }
    throw err
  } finally {
...
export async function checkUpdates(pluginsDir: string, appVersion?: string): Promise<Array<{ id: string; name?: string; installedVersion?: string; newVersion: string; downloadUrl: string }>> {
