...
export function moveMouse(dx: number, dy: number): void {
  const rdx = Math.round(dx); const rdy = Math.round(dy)
  if (rdx === 0 && rdy === 0) return
  if (OS === PLATFORMS.LINUX)   executeCommand(`xdotool mousemove_relative -- ${rdx} ${rdy}`)
  else if (OS === PLATFORMS.DARWIN) executeCommand(`osascript -e "tell application \\"System Events\\" to set mouseLocation of the process \\"Finder\\" to {0,0}"` )
  // macOS relative move via cliclick if available
}

export function clickMouse(button: 1 | 2 | 3): void {
  if (OS === PLATFORMS.LINUX)       executeCommand(`xdotool click ${button}`)
  else if (OS === PLATFORMS.DARWIN) executeCommand(`osascript -e 'tell application "System Events" to ${button === 1 ? 'click at {get mouseLocation}' : 'right click at {get mouseLocation}'}'`)
  else                              executeCommand(`powershell -c "Add-Type -Assembly System.Windows.Forms; [System.Windows.Forms.SendKeys]::SendWait(' ')"`)
}

export function scrollMouse(dy: number): void {
  const steps = Math.round(dy / 10)
  if (steps === 0) return
  if (OS === PLATFORMS.LINUX)       executeCommand(`xdotool click --repeat ${Math.abs(steps)} ${dy < 0 ? 4 : 5}`)
  else if (OS === PLATFORMS.DARWIN) executeCommand(`osascript -e 'tell application "System Events" to scroll ${Math.abs(steps)} lines ${dy < 0 ? 'up' : 'down'}'`)
}
