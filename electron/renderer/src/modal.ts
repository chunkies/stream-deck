...
// ── Plugin action select helper ────────────────────────

const TRACKPAD_GESTURES = ['swipeLeft', 'swipeRight', 'swipeUp', 'swipeDown', 'doubleTap', 'longPress', 'twoFingerTap', 'pinchIn', 'pinchOut'] as const

function populatePluginActionSelect(selId: string, selectedKey = ''): void {
  ...
  for (const plugin of state.loadedPlugins) {
    for (const action of (plugin.actions ?? [])) {
      ...
      opt.textContent = `${plugin.name}: ${action.label}`
      selEl.appendChild(opt)
    }
  }
  if (selectedKey) selEl.value = selectedKey
}

// ── Plugin-tile dropdown helpers ───────────────────────

/** Replace the plain text inputs in the plugin-tile section with smart selects.
 *  Idempotent — safe to call multiple times (skips if selects are already present). */
function buildPluginTileDropdowns(): void {
  const existingPluginInput = document.getElementById('ptile-plugin-id')
  if (existingPluginInput && existingPluginInput.tagName !== 'SELECT') {
    const pluginSel = document.createElement('select')
    pluginSel.id        = 'ptile-plugin-id'
    pluginSel.className = existingPluginInput.className
    existingPluginInput.replaceWith(pluginSel)
  }

  const existingEventInput = document.getElementById('ptile-event')
  if (existingEventInput && existingEventInput.tagName !== 'SELECT') {
    const eventSel = document.createElement('select')
    eventSel.id        = 'ptile-event'
    eventSel.className = existingEventInput.className
    existingEventInput.replaceWith(eventSel)
  }

  const pluginSelect = sel('ptile-plugin-id')
  ...
  if (!state.loadedPlugins.length) {
    ...
    opt.disabled = true
    opt.selected = true
    opt.textContent = 'No plugins installed'
  ...
  // Clone to remove prior listeners before adding new ones
  const freshPluginSel = pluginSelect.cloneNode(true) as HTMLSelectElement
  pluginSelect.replaceWith(freshPluginSel)
  freshPluginSel.addEventListener('change', () => populatePtileEventSelect(freshPluginSel.value))
}

/** Populate the event select with the actions of the given plugin. */
function populatePtileEventSelect(pluginId: string): void {
  const eventSelect = sel('ptile-event')
  eventSelect.innerHTML = ''

  const plugin = state.loadedPlugins.find(p => p.id === pluginId)
  ...
  const tileLike = plugin.actions.filter(a => !a.componentType || a.componentType === 'tile')
  const others   = plugin.actions.filter(a => a.componentType && a.componentType !== 'tile')

  for (const action of [...tileLike, ...others]) {
    const opt = document.createElement('option')
    opt.value       = action.key
...
export function setCompType(type: string): void {
  ...
  for (const t of ['button', 'switch', 'slider', 'knob', 'tile', 'voice', 'folder', 'counter', 'clock', 'stopwatch', 'countdown', 'trackpad']) {
...
export function openModal(pageIdx: number, compId: string | null, col?: number, row?: number): void {
  ...
  if (uiType === 'switch') {
    ...
    chk('ea-active-default').checked = !!comp?.activeDefault
    ...
    if (tAt === 'plugin') populatePluginActionSelect('t-plugin-action', ta?.type === 'plugin' ? ta.pluginKey : '')
  }
  ...
  if (uiType === 'slider') {
    ...
    if (sAt === 'plugin') populatePluginActionSelect('s-plugin-action', sa?.type === 'plugin' ? sa.pluginKey : '')
  }
  ...
  if (uiType === 'knob') {
    ...
    if (kAt === 'plugin') populatePluginActionSelect('k-plugin-action', ka?.type === 'plugin' ? ka.pluginKey : '')
  }
  ...
  if (uiType === 'tile') {
    ...
    buildPluginTileDropdowns()
    const pluginSel = sel('ptile-plugin-id')
    if (comp?.pluginTileId) {
      pluginSel.value = comp.pluginTileId
      pluginSel.dispatchEvent(new Event('change'))
    } else if (pluginSel.options.length > 0 && !pluginSel.disabled) {
      pluginSel.selectedIndex = 0
      pluginSel.dispatchEvent(new Event('change'))
    }
    if (comp?.pluginTileEvent) sel('ptile-event').value = comp.pluginTileEvent
  ...
  if (uiType === 'trackpad') {
    ...
    const ta = comp?.trackpadActions ?? {}
    for (const g of TRACKPAD_GESTURES) {
      const existing = ta[g as keyof typeof ta]
      populatePluginActionSelect(`tp-gest-${g}`, existing?.type === 'plugin' ? existing.pluginKey : '')
    }
  }
...
export function saveModal(): void {
  ...
  if (state.currentCompType === 'switch' || state.currentCompType === 'toggle') {
    ...
    switch (tAt) {
      ...
      case 'plugin':   tAction = { type: 'plugin',   pluginKey: sel('t-plugin-action').value, params: {} }; break
    ...
    fields = {
      ...
      activeDefault: chk('ea-active-default')?.checked ?? false,
      action:       tAction,
    }
  }
  ...
  if (state.currentCompType === 'slider') {
    ...
    switch (sAt) {
      ...
      case 'plugin':   sAction = { type: 'plugin',   pluginKey: sel('s-plugin-action').value, params: {} }; break
  ...
  if (state.currentCompType === 'knob') {
    ...
    switch (kAt) {
      ...
      case 'plugin':   kAction = { type: 'plugin',   pluginKey: sel('k-plugin-action').value, params: {} }; break
  ...
  if (state.currentCompType === 'tile') {
    fields = {
      ...
      pluginTileId:    sel('ptile-plugin-id').value.trim() || undefined,
      pluginTileEvent: sel('ptile-event').value.trim()     || undefined,
  ...
  if (state.currentCompType === 'trackpad') {
    const trackpadActions: Partial<Record<string, { type: 'plugin'; pluginKey: string; params: Record<string, unknown> }>> = {}
    for (const g of TRACKPAD_GESTURES) {
      const selEl = document.getElementById(`tp-gest-${g}`) as HTMLSelectElement | null
      if (selEl?.value) trackpadActions[g] = { type: 'plugin', pluginKey: selEl.value, params: {} }
    }
    fields = {
      ...
      trackpadActions:       Object.keys(trackpadActions).length > 0 ? trackpadActions as Component['trackpadActions'] : undefined,
    }
  }
