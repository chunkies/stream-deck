...
import { createComponentAtCell, createWidgetAtCell } from './components'
...
export function renderGrid(): void {
  ...
  for (let r = 1; r <= rows; r++) {
    for (let c = 1; c <= cols; c++) {
      ...
      ghost.addEventListener('drop', (e: DragEvent) => {
        ...
        let data: { widgetDrop?: boolean; pluginId?: string; widgetKey?: string; compType?: ComponentType; pluginKey?: string | null; label?: string; options?: Partial<Component> }
        try { data = JSON.parse(e.dataTransfer.getData('application/json')) } catch { return }
        if (data.widgetDrop) {
          const plugin = state.loadedPlugins.find(p => p.id === data.pluginId)
          const widget = plugin?.widgets?.find(w => w.key === data.widgetKey)
          if (plugin && widget) createWidgetAtCell(plugin, widget, c, r)
          return
        }
        createComponentAtCell(data.compType!, data.pluginKey ?? null, data.label ?? '', c, r, data.options ?? {})
      })
  ...
  for (const comp of (page.components ?? [])) {
    ...
    switch (comp.componentType) {
      ...
      case 'tile': {
        const isPlugin = !!comp.pluginTileId
        const badge    = isPlugin ? 'plugin' : 'tile'
        const cmd      = isPlugin
          ? `${comp.pluginTileId}:${comp.pluginTileEvent || ''}`
          : (comp.pollCommand || '').substring(0, 26)
        card.innerHTML = `<div class="cell-type-badge">${badge}</div><div class="cell-tile-cmd"></div><div class="cell-label"></div><div class="resize-handle"></div>`
        card.querySelector<HTMLElement>('.cell-tile-cmd')!.textContent = cmd
        card.querySelector<HTMLElement>('.cell-label')!.textContent    = comp.label || ''
        break
      }
      ...
      case 'trackpad':
        card.innerHTML = '<div class="cell-type-badge">trackpad</div><div class="cell-icon">🖱️</div><div class="cell-label"></div><div class="resize-handle"></div>'
        card.querySelector<HTMLElement>('.cell-label')!.textContent = comp.label || 'Trackpad'
        break
