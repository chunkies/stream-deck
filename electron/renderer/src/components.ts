...
import type { Component, ComponentType, PluginMeta, PluginWidget } from '../../shared/types'
...
export function compDefaults(compType: ComponentType): Partial<Component> {
  ...
  switch (compType) {
    ...
    case 'tile':        return { ...base, color: '#0f172a', pollCommand: '', pollInterval: 5, pluginTileId: '', pluginTileEvent: '', pluginTileField: 'value' }
    ...
    case 'spotify':     return { ...base, color: '#0f172a' }
  }
}
...
export function createWidgetAtCell(plugin: PluginMeta, widget: PluginWidget, col: number, row: number): void {
  const page = adminPages()[adminIdx()]
  const id   = `c-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`
  const comp: Component = {
    ...
    col,
    row,
    colSpan:         widget.defaultColSpan ?? 2,
    rowSpan:         widget.defaultRowSpan ?? 1,
    componentType:   'tile',
    label:           widget.label,
    color:           '#0f172a',
    pluginTileId:    plugin.id,
    pluginTileEvent: widget.key,
    pluginTileField: widget.field ?? 'value',
  }
  page.components.push(comp)
  pushConfig()
  renderGrid()
...
export function renderComponentPanel(): void {
  ...
  if (state.loadedPlugins.length) {
    ...
    for (const plugin of state.loadedPlugins) {
      const hasActions = plugin.actions?.length > 0
      const hasWidgets = (plugin.widgets?.length ?? 0) > 0
      if (!hasActions && !hasWidgets) continue

      const actionItems: CpItem[] = (plugin.actions ?? []).map(a => ({
      ...
      const hdr = document.createElement('div')
      hdr.className = 'cp-section-title cp-collapsible'
      const isCollapsed = state.cpCollapsed.has(plugin.id)
      const hdrTitle = document.createElement('span')
      hdrTitle.textContent = plugin.name
      const hdrChevron = document.createElement('span')
      hdrChevron.className = 'cp-chevron'
      hdrChevron.textContent = isCollapsed ? '▸' : '▾'
      hdr.appendChild(hdrTitle)
      ...
      hdr.addEventListener('click', () => {
        ...
        hdr.querySelector<HTMLElement>('.cp-chevron')!.textContent = collapsed ? '▾' : '▸'
        if (collapsed) state.cpCollapsed.delete(plugin.id)
        else state.cpCollapsed.add(plugin.id)
      })

      // Widgets first (pre-configured tiles) then actions (manual-config items)
      for (const widget of (plugin.widgets ?? [])) {
        const itemEl = document.createElement('div')
        itemEl.className = 'cp-item cp-widget-item'
        itemEl.draggable = true
        const iconSpan = document.createElement('span')
        iconSpan.className = 'cp-icon'
        iconSpan.textContent = widget.icon ?? plugin.icon ?? '📊'
        const nameSpan = document.createElement('span')
        nameSpan.className = 'cp-name'
        nameSpan.textContent = widget.label
        const typeSpan = document.createElement('span')
        typeSpan.className = 'cp-type'
        typeSpan.textContent = 'widget'
        itemEl.appendChild(iconSpan)
        itemEl.appendChild(nameSpan)
        itemEl.appendChild(typeSpan)
      ...
      // Action items (open modal on drop for manual config)
      ...
      pluginsBody.appendChild(section)
    }
