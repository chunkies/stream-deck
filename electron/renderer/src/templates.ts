...
export function openTemplateStore(): void {
  renderTemplateStore()
  const overlay = document.getElementById('template-store')
  if (overlay) overlay.style.display = ''
}

export function closeTemplateStore(): void {
  const overlay = document.getElementById('template-store')
...
function renderTemplateStore(): void {
  const body = document.getElementById('template-store-body')
  if (!body) return
  body.innerHTML = ''

  const grid = document.createElement('div')
  grid.style.cssText = 'display:grid;grid-template-columns:repeat(2,1fr);gap:12px;padding:4px 0'

  for (const template of TEMPLATES) {
    const card = document.createElement('div')
    card.style.cssText = 'background:#1e293b;border:1px solid #334155;border-radius:8px;padding:14px;display:flex;flex-direction:column;gap:8px'
    ...
    descEl.style.cssText = 'margin:0;font-size:12px;color:#94a3b8;line-height:1.4'
    descEl.textContent = template.description

    const addBtn = document.createElement('button')
    addBtn.className = 'btn-primary'
    addBtn.style.cssText = 'margin-top:auto;font-size:12px'
    addBtn.textContent = 'Add to Layout'

    addBtn.addEventListener('click', () => {
      addTemplateToConfig(template)
